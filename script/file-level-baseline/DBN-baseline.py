...
def train_model(dataset_name):
    ...
    dbn_clf.fit(features, train_label)  # 训练

    dbn_features = dbn_clf.transform(features)

    rf_clf = RandomForestClassifier(n_jobs=24)  # 随机森林分类器

    rf_clf.fit(dbn_features, train_label)  # 训练

    pickle.dump(dbn_clf, open(actual_save_model_dir + dataset_name + '-DBN.pkl', 'wb'))
    pickle.dump(rf_clf, open(actual_save_model_dir + dataset_name + '-RF.pkl', 'wb'))
...
def predict_defective_files_in_releases(dataset_name):
    ...
    dbn_clf = pickle.load(open(actual_save_model_dir + dataset_name + '-DBN.pkl', 'rb'))
    rf_clf = pickle.load(open(actual_save_model_dir + dataset_name + '-RF.pkl', 'rb'))
    ...
    for rel in eval_rel:
        ...
        # 计算recall
        recall = recall_score(df['file-level-ground-truth'], df['prediction-label'])
        print('recall:', recall)
