...
from sklearn.metrics import roc_auc_score, matthews_corrcoef, f1_score, accuracy_score, precision_score
...
arg.add_argument('-target_epochs', type=str, default='11', help='which epoch of model to load')
...
def predict_defective_files_in_releases(dataset_name, target_epochs=6):
    ...
    for rel in eval_rels:
        ...
        # 计算accuracy
        accuracy = accuracy_score(df['file-level-ground-truth'], df['prediction-label'])
        ...
        print('AUC', AUC)
        # 计算MCC
        MCC = matthews_corrcoef(df['file-level-ground-truth'], df['prediction-label'])
        print('MCC', MCC)
